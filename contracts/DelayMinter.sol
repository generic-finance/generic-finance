...
contract DelayMinter is Governable {
  ...
  uint256 public lpRatio =        80; // 80% to holders
  uint256 public grantsRatio =    10; // not used, but keeping for clarity
  ...
  event NewGrants(address grants);
  event NewOperator(address operator);

  constructor(address _storage, address _token, uint256 _delay, address _grants, address _operator)
  Governable(_storage) public {
    ...
    grants = _grants;
    require(grants != address(0), "grants not set");
    operator = _operator;
  ...
  function executeMint(uint256 _id) public onlyGovernance {
    ...
    uint256 toGrants = amount.sub(toTarget).sub(toOperator);
    
    ERC20Mintable(token).mint(target, toTarget);
    ERC20Mintable(token).mint(operator, toOperator);
    ERC20Mintable(token).mint(grants, toGrants);
  ...
  function setGrants(address _grants) public onlyGovernance {
    require(_grants != address(0), "Address should not be 0");
    grants = _grants;
    emit NewGrants(_grants);
  }
